...
hist_make <- function (n, exp_prob, get_hist, distr = 'binomlog'){
  ...
  if (distr == 'binomlog'){
    if (n == 0){
      ...
      while (right - left > 2) {
        ...
        # print(nLeft)
        qLeft <- optim(c(0.5, 1 / nLeft), function(x) m_log_lik_binomlog(sam, p = x[2], n = nLeft, q = x[1]))
        qRight <- optim(c(0.5, 1 / nRight), function(x) m_log_lik_binomlog(sam, p = x[2], n = nRight, q = x[1]))
      ...
      minValue <- optim(c(0.5, 1 / (left + 1)), function(x) m_log_lik_binomlog(sam, p = x[2], n = left, q = x[1]))
      # q <- minValue$minimum
      q <- minValue$par[1]
      p <- minValue$par[2]
      n <- left
      for (i in (left + 1):right){
        ...
        value <- optim(c(0.5, 1 / i), function(x) m_log_lik_binomlog(sam, p = x[2], n = i, q = x[1]))
      ...
      p <- (var_sam / mean_sam * log(1 - q) * (1 - q) - log(1 - q)) / q
    }
    ...
    if (n == 0){
      ...
      minValue <- optim(c(0.5, 1 / (left + 1)), function(x) m_log_lik_logbinom(sam, q = x[1], p = x[2], n = left))
      # q <- minValue$minimum
      q <- minValue$par[1]
      p <- minValue$par[2]
      n <- left
    ...
    q <- optim(c(0.5, 1), function(x) m_log_lik_logpois(sam, q = x[1], lambda = x[2]))
    lambda <- q$par[2]
    q <- q$par[1]
    res <- c(lambda, q)
  } else if (distr == 'nbinom'){
    q <- optim(c(0.5, 1), function(x) m_log_lik_nbinom(sam, q = x[1], n = x[2]))
    n <- q$par[2]
    q <- q$par[1]
    res <- c(n, q)
    ...
    lambda <- optimize(function(x) m_log_lik_pois(sam, lambda = x), c(0, 1000))$minimum
    res <- c(lambda)
  }
  ...
  for (i in 0:(length(exp_prob) - 2)){
    if (distr == 'binomlog'){
      ...
      prob <- c(prob, dpois(i, lambda = res[1]))
    } 
  }
  ...
  if (get_hist){
    hist.default(sam, probability = TRUE, breaks = seq(-0.5, max_el_x + 0.5, 1), xlim = c(-0.5, max_el_x + 0.5), ylim = c(0, max_el_y), xlab = "Выборка", ylab = "Вероятность", main = "")
...
```{r}
par(mfrow = c(1, 3))
plot(x = seq_LBR, y = q_LBR, xlab = "Размер выборки", ylab = "Оценка параметра q логарифма")
abline(h = 0.4, lty = "dashed")
legend(x = "topright", legend = "Истинное значение", lty = 2, col = 1, lwd = 2)
plot(x = seq_LBR, y = n_LBR, xlab = "Размер выборки", ylab = "Оценка параметра n бинома")
abline(h = 6, lty = "dashed")
legend(x = "topright", legend = "Истинное значение", lty = 2, col = 1, lwd = 2)
plot(x = seq_LBR, y = p_LBR, xlab = "Размер выборки", ylab = "Оценка параметра p бинома")
...
```{r warning=FALSE}
samBLR <- rbinomlog(1000, 8, 0.25, log_prepering(0.2))
histBLR <- hist(as.numeric(samBLR), probability = TRUE, breaks = seq(-0.5, max(samBLR) + 0.5, 1), xlim = c(-0.5, max(samBLR) + 0.5), plot = TRUE, xlab = "Выборка", ylab = "Вероятность", main = "")
res <- hist_make(0, as.numeric(histBLR$counts), get_hist = TRUE, 'binomlog')
funcMP_3D(histBLR$counts, inter_par2 = c(1, 100, 100))
res
```
...
```{r warning=FALSE}
samLPR <- rlogpois(1000, log_prepering(0.25), 3.45)
histLPR <- hist(as.numeric(samLPR[1:100]), probability = TRUE, breaks = seq(-0.5, max(samLPR) + 0.5, 1), xlim = c(-0.5, max(samLPR) + 0.5), plot = TRUE)
...
df.PR <- data.frame(lambd = rep(0, 19), p_value = rep(0, 19))
```
...
```{r warning=FALSE}
res <- hist_make(0, as.numeric(m[18,]), get_hist = TRUE, 'binomlog')
res
...
funcMP_3D(as.numeric(m[7,]), distr = 'logbinom', inter_par2 = c(1, 20, 100))
...
plot(seq(0, 45, 5), df.LBR$n[1:10], type = "l", col = "blue", ylim = c(0, max(df.LBR)), ylab = "Параметр n", xlab = "Доза облучения, Гр", main = "Black - in vitro; Blue - in vivo")
...
```{r}
WordSample <- read.csv("./output.csv")
...
histWord <- hist(as.numeric(word), probability = TRUE, breaks = seq(-0.5, max(word) + 0.5, 1), xlim = c(-0.5, max(word) + 0.5), main = "", xlab = "Значения выборки", ylab = "Вероятности")
...
res
res <- hist_make(0, histWord$counts, get_hist = TRUE, distr = 'logpois')
res
